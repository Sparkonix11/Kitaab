<script setup>
import { fetchAllApprovedBooks, revokeBook } from '@/services/operations/bookIssuanceAPI';
            ...
            <div v-for="book in issuedBooks" :key="book.id">
                <div class="card">
                    ...
                    <div class="card-hover-content">
                        ...
                        <button class="button" v-on:click="revokeBookHandler(book.book_id, book.user_id)">Revoke Access</button>
                    </div>
                    <h3>{{ book.book_title }}</h3>by
                    <h4>Issued to - {{ book.user_name }}</h4>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        async fetchIssuedBooks(){
            try {
                const { success, responseData } = await fetchAllApprovedBooks();
                if (success) {
        ...
        async revokeBookHandler(bookId, userId){
            try {
                const { success, responseData } = await revokeBook(bookId, userId);
                if (success) {
                    this.issuedBooks = responseData;
                } else {
                    console.error('Failed to revoke Access of the book');
                }
            } catch (error) {
